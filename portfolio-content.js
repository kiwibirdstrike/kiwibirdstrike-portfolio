window.PORTFOLIO_DATA = {
  ...
  featuredProjects: [
    {
      ...
      technicalDetails: {
        architecture: [
          ...
          "문서 처리 계층은 공고에 포함된 여러 첨부파일 중 실제 참가자격이 담길 가능성이 높은 대표 공고문을 먼저 고릅니다. 파일명, 확장자, 공고명과 공고번호 일치 여부를 점수화하고 시방서·도면·과업지시서처럼 목적이 다른 문서는 감점합니다.",
          "해석 계층은 원문에서 조건을 넓게 회수하는 Stage 1과 회수한 근거를 면허·인증·실적·인력·지역·제출서류로 정리하는 Stage 2로 분리했습니다. 누락과 과잉 추출을 서로 다른 단계에서 관리하기 위한 설계입니다.",
          "서비스 계층에서는 파싱 결과를 parsed_tenders라는 중간 표현으로 고정한 뒤 회사 프로필과 비교합니다. 프런트엔드, 판정 엔진, 검색 기능이 원문을 직접 해석하지 않고 동일한 구조화 결과를 사용하도록 경계를 만들었습니다."
        ],
        dataFlow: [
          "나라장터 원본 JSON을 수집하고 취소·정정·변경 공고와 수의계약 등 분석 대상이 아닌 입력을 먼저 제외합니다.",
          "첨부 후보를 점수화해 핵심 공고문 한 건을 선택하고 PDF·HWP·HWPX에서 본문, 페이지, 순서가 유지된 텍스트 블록을 추출합니다.",
          "Stage 1 에이전트가 참가 가능성에 영향을 주는 문장을 범주별로 탐색합니다. 애매한 조건은 버리지 않고 근거 후보로 보존한 뒤 중앙 감사 단계에서 누락 여부를 검사합니다.",
          "Stage 2 에이전트가 근거 후보를 고정된 JSON 스키마에 배치하고, workflow·overview 메타데이터를 추가해 서비스가 읽을 수 있는 parsed_tenders 레코드로 저장합니다.",
          "판정 단계에서 구조화된 요구조건과 회사의 면허·인증·지역·실적·인력 정보를 비교하고 참가 가능, 조건부 가능, 참가 불가, 전문가 검토 필요로 결과를 정리합니다."
        ],
        implementation: [
          "run_staged_tender_pipeline은 동일한 공고 집합을 수집, 첨부 다운로드, 파싱, DB 저장 단계로 순차 통과시킵니다. 단계별 재실행 옵션과 처리시간 기록을 두어 실패 지점과 성능 병목을 분리했습니다.",
          "score_document와 select_core_candidate는 첨부문서의 확장자와 파일명 신호를 조합해 대표 문서를 선택합니다. 단순히 첫 번째 첨부를 사용하는 경우 발생하던 시방서·양식 오선택을 줄이기 위한 전처리입니다.",
          "extract_from_path와 parse_tender_document는 PDF와 HWP 계열 문서를 공통 meta 구조로 변환합니다. summarize_from_meta는 추출 텍스트를 JSON Schema 출력으로 제한해 후속 서비스가 필드 이름과 자료형을 예측할 수 있도록 했습니다.",
          "Stage 1은 자격·참여제약·제출서류·평가조건·공급범위 등 담당 범주를 나눈 전문 에이전트가 근거를 수집하고, 오케스트레이터가 결과를 병합합니다. 감사에서 실패한 공고와 범주만 다시 추론해 전체를 반복 실행하지 않도록 구성했습니다.",
          "DecisionEngine은 LLM 판정 결과를 그대로 사용하지 않습니다. 허용되지 않은 상태값을 검토 필요로 바꾸고, 누락된 비교 항목은 규칙 기반 평가로 보강하며, 공동수급 조건처럼 해석이 흔들릴 수 있는 항목은 원본값을 유지한 채 최종 상태를 다시 계산합니다."
        ],
        decisions: [
          "가장 큰 위험을 참가 가능한 공고를 놓치는 기회 손실로 정의했습니다. 따라서 Stage 1에서는 정밀도보다 재현율을 우선하고, 과잉 후보는 Stage 2에서 제거하도록 역할을 분리했습니다.",
          "원본 JSON과 첨부문서는 파일을 기준 데이터로 유지하고, DB에는 서비스가 실제 사용하는 구조화 결과와 판정 결과를 저장했습니다. 원문 보존과 서비스 조회 성능을 동시에 확보하기 위한 선택입니다.",
          "모든 첨부를 LLM에 입력하면 비용과 처리시간이 크게 늘어나므로 대표 공고문 한 건을 선택했습니다. 대신 선택 이유와 점수를 남겨 잘못된 문서가 선택된 경우 원인을 추적할 수 있게 했습니다.",
          "면허나 실적 결과만 저장하지 않고 source_sentences를 함께 보존했습니다. 사용자가 판정 결과를 원문 근거와 대조하고, 오류가 발생했을 때 어느 문장에서 시작됐는지 확인할 수 있도록 하기 위함입니다.",
          "모델 출력 뒤에 규칙 기반 검증과 정규화 단계를 추가했습니다. 생성 모델의 형식 오류나 필드 누락이 곧바로 참가 가능 여부의 오류로 이어지지 않도록 한 안전장치입니다."
        ],
        experiments: [
          "Stage 1은 공고 100건을 직접 읽어 만든 605개 정답 행으로 평가했습니다. 이 표본에서는 605개 근거를 모두 포착해 row-key recall 100%와 누락 0건을 확인했습니다.",
          "추가로 공고 1,000건을 자동 감사해 모든 결과가 필수 파일과 구조 조건을 충족하는지 확인했습니다. 이 수치는 정확도 100%가 아니라 대량 처리 파이프라인의 구조적 안정성 지표로 구분했습니다.",
          "Stage 2는 공고 30건의 API 비교 가능 필드 87개 중 76개가 일치해 87.36%를 기록했습니다. 제목은 90%, 공동수급은 100%였지만 지역 제한은 73.33%로 낮아 개선이 필요한 필드를 구체적으로 특정했습니다.",
          "오류 분석에서는 대표 첨부문서가 아닌 파일을 선택하거나, 의미는 포착했지만 면허·인력·실적 필드에 잘못 배치하는 문제가 확인됐습니다. 이후 첨부 점수화와 단계별 감사·수리 구조를 보강하는 근거로 사용했습니다.",
          "성과 수치는 검증 범위를 함께 표기했습니다. Stage 1의 100%는 100건·605개 정답 행에 한정하고, Stage 2의 87.36% 역시 제목·지역·공동수급의 제한된 비교 필드 결과로만 해석했습니다."
        ]
      },
      links: [
  ...
  timeline: [
    ...
    { date: "2025.07", title: "Scientific Reports 논문", note: "Penalized B-spline 연구 공동저자", track: "bio", category: "논문" },
    { date: "2025.09", title: "공공체육시설 입지 분석", note: "시설 접근성을 고려한 최적 입지 선정 · 우수상", track: "analytics", category: "공모전" },
    { date: "2025.10", title: "KAMP 제조데이터 분석대회", note: "공정 예측 모델과 현장용 웹 분석 도구 개발", track: "analytics", category: "공모전" },
    { date: "2025.11", title: "TVspline", note: "스플라인 R 패키지 제작 및 관리", track: "analytics", category: "연구 개발" },
    { date: "2026.01", title: "AAAI 학회 참석", note: "멀티모달 AI 연구 동향과 모델 구조 탐색", track: "agent", category: "학회" },
    { date: "2026.01", title: "결측치 대치 방법론 연구", note: "예측 성능과 통계적 추론의 신뢰성 비교", track: "bio", category: "연구" },
    { date: "2026.02", title: "연구실 예약 PWA", note: "수기 예약 절차를 공용 예약 서비스로 전환", track: "agent", category: "서비스 개발" },
    { date: "2026.03", title: "JOBIS", note: "공공조달 공고 적격성 판정 서비스 설계 및 구축", track: "agent", category: "서비스 개발" },
    { date: "2026.03", title: "AutoResearch", note: "실험 결과와 분석 지식을 재사용하는 연구 자동화 구조 설계", track: "agent", category: "공모전" },
    { date: "2026.03", title: "ILR Polygram", note: "조성자료를 위한 로지스틱 회귀 방법론 연구", track: "bio", category: "연구" },
    { date: "2026.03", title: "WorldQuant", note: "퀀트 모델링과 알파 검증 방법 학습", track: "agent", category: "대회" },
    { date: "2026.06", title: "자동투자 포트폴리오", note: "백테스트, 위험 관리와 주문 계획을 연결한 투자 시스템 구축", track: "agent", category: "개인 프로젝트" }
  ],
  otherProjects: [
    {
